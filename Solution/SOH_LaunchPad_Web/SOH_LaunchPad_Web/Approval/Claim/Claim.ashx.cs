...
namespace SOH_LaunchPad_Web.Approval.Claim
{
    /// <summary>
    /// Summary description for Claim
    /// </summary>
    public class Claim : HttpTaskAsyncHandler, IRequiresSessionState
    {
        public override async Task ProcessRequestAsync(HttpContext context)
        {
            if (HttpContext.Current.Request.HttpMethod == "POST")
            {
                try
                {
                    ...
                    if (action == "list")
                    {
                        var result = await GenericRequest.Post(Common.ApprovalWSEndpointUrl + "claim/list.ashx", new StringContent(input));
                    ...
                    {
                        var result = await GenericRequest.Post(Common.ApprovalWSEndpointUrl + "claim/approve.ashx", new StringContent(input));
                        if (result.Status == RequestResult.ResultStatus.Failure)
                        {
                            context.Response.StatusCode = 400;
                            context.Response.StatusDescription = result.GetErrmsgTrim();
