...
using SOH_LaunchPad_Approval.pn.src;
using System;
...
namespace SOH_LaunchPad_Approval.pn
{
    /// <summary>
        ...
        public override async Task ProcessRequestAsync(HttpContext context)
        {
            if (HttpContext.Current.Request.HttpMethod == "POST")
            {
                ...
                try
                {
                    ...
                    DAO.PNApproveRst rst = JsonConvert.DeserializeObject<DAO.PNApproveRst>(data);

                    DAO dao = new DAO();
                    if (dao.CheckReleaseAuth(approverid, rst.SO, rst.Vendor) == false)
                    {
