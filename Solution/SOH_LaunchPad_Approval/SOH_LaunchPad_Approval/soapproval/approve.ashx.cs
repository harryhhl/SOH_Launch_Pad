...
namespace SOH_LaunchPad_Approval.soapproval
{
    /// <summary>
        ...
        public override async Task ProcessRequestAsync(HttpContext context)
        {
            if (HttpContext.Current.Request.HttpMethod == "POST")
            {
                ...
                try
                {
                    ...
                    string[] tmpSOList = data.Split(new char[] { ',' });
                    List<SOApproveRst> rst = new List<SOApproveRst>();
                    foreach(string so in tmpSOList)
                    {
                        SOApproveRst s = new SOApproveRst();
                        s.Vbeln = so.Trim();
                        rst.Add(s);
                    }
