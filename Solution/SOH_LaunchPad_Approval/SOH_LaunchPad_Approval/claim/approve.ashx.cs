...
using SOH_LaunchPad_Approval.claim.src;
using System;
...
namespace SOH_LaunchPad_Approval.claim
{
    /// <summary>
        ...
        public override async Task ProcessRequestAsync(HttpContext context)
        {
            if (HttpContext.Current.Request.HttpMethod == "POST")
            {
                ...
                try
                {
                    ...
                    DAO.ClaimApproveRst rst = JsonConvert.DeserializeObject<DAO.ClaimApproveRst>(data);

                    DAO dao = new DAO();
                    if (dao.CheckReleaseAuth(approverid, rst.Type, rst.Doc) == false)
                    {
                        throw new Exception("You have No Access Rights for this Document! ");
                    }
